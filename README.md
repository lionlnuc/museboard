...
Museboard 没有后端、账号系统或遥测服务。画板文档与图片资源保存在浏览器的 IndexedDB 中，兼容副本保存在 localStorage 中。清除浏览器站点数据会同时删除未导出的本地画板，因此重要内容应定期导出为 JSON。
...
项目坚持纯前端和本地优先，因此当前不包含实时协作、云同步、账号系统或在线模板市场。不同浏览器之间的数据不会自动同步。
